...
def binary_mechanism_geo(epsilon, df, result_df, theta):
    ...
    for t in range(t_last, t_last+num_rows):
        ...
        for region in regional_values:    
            ...
            regional_data_df.at[t-1, region] = laplace_mechanism(regional_values[region],ai(i, theta)*3,epsilon)
 
        # Add noise to DK value
        regional_data_df.at[t-1, "DK"] = laplace_mechanism(DK, ai(i, theta)*3,epsilon)
