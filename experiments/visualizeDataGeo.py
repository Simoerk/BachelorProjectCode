...
# Experiment that visualize the data from the NumMunUnbGeo and real data

# Read the CSV files into DataFrames
actual_df = pd.read_csv('results/regional_consumption_sums.csv')
result_df = pd.read_csv('results/NumMunUnbGeo_noisy_result.csv')
...
# Function that plots the consumption
...
def calculate_stepwise_difference(df):
    """
    Calculate the difference in consumption for each municipality between consecutive timesteps.
    """
    # Ensure the DataFrame is sorted by HourDK
    df = df.sort_values(by='HourDK')

    # Calculate differences for each municipality column
    municipality_columns = df.columns[1:]  # Exclude the HourDK column
    differences = df[municipality_columns].diff().fillna(0)

    # Add the HourDK column back to the differences DataFrame
    differences['HourDK'] = df['HourDK']

    return differences

def plot_single_timestep_difference(df, timestep):
    """
    Plot the contribution of a single timestep for each municipality.
    """
    # Calculate the differences
    difference_df = calculate_stepwise_difference(df)

    # Extract data for the given timestep
    timestep_data = difference_df[difference_df['HourDK'] == timestep]

    if timestep_data.empty:
        print(f"No data available for the given timestep: {timestep}")
        return

    # Drop the HourDK column for plotting
    timestep_data = timestep_data.drop(columns=['HourDK'])

    # Plot the data
    plt.figure(figsize=(12, 6))
    plt.bar(timestep_data.columns, timestep_data.iloc[0])
    plt.xlabel('Municipality')
    plt.ylabel('Consumption Difference (kWh)')
    plt.title(f'Consumption Difference at Timestep {timestep}')
...
# Convert 'HourDK' to datetime for proper filtering
actual_df['HourDK'] = pd.to_datetime(actual_df['HourDK'])
result_df['HourDK'] = pd.to_datetime(result_df['HourDK'])

# Print column names to verify
print("Actual DataFrame columns:", actual_df.columns)
print("Result DataFrame columns:", result_df.columns)

# Choose a specific timestep to plot
timestep_to_plot = '2024-02-21 03:00:00'

# Convert timestep_to_plot to datetime
timestep_to_plot = pd.to_datetime(timestep_to_plot)

plot_single_timestep_difference(result_df, timestep_to_plot)
