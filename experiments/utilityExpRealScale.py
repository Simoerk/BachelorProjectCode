...
for epsilon in epsilons:
    ...
    # Re-initialize the dataframe pairs
    dataframe_pairs = [
    ...
    for name, noisy_df, real_df in dataframe_pairs:
        ...
        if name != 'Bin': # Don't scale down bin as it is already binary
...
for name, datasets in name_data.items():
    ...
    for epsilon, data in datasets.items():
        ...
        # Plot each epsilon density and store the line object
        line, = plt.plot(x, y, label=f'Epsilon = {epsilon}')
        
        # Mark the furthest negative and positive values with matching color
        plt.axvline(np.min(data), color=line.get_color(), linestyle='dashed', linewidth=1)
        plt.axvline(np.max(data), color=line.get_color(), linestyle='dashed', linewidth=1)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'results/RealError/error_density_{name}.png')  # Save the plot as a PNG file
    plt.close()
